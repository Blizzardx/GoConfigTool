...
func parserLine_BasicItem_CommonConfig(lineContent []string) (*BasicItem_CommonLineInfo, error) {
	...
	err = common.Parser_int32(lineContent[columnIndex], &line.ConsumeItem)
	if nil != err {
		return nil, errors.New(" column " + strconv.Itoa(columnIndex) + " named ConsumeItem " + err.Error())
	}

	err = common.CheckValueLimit_int32(line.ConsumeItem, "", "")
	if err != nil {
		return nil, errors.New(" column " + strconv.Itoa(columnIndex) + " named ConsumeItem error on check min and max" + err.Error())
	}
